    ...
    <aside className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex flex-col overflow-y-auto shadow-lg z-20">
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-blue-900 dark:text-blue-200">FinCore Banking</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">Sistema Financiero Moderno</p>
          </div>
        </div>
      </motion.div>
      ...
      {user && (
        ...
        >
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              <div className="font-medium text-sm text-slate-800 dark:text-slate-100">{user.nombreCompleto || user.username}</div>
      ...
      {cuentas.length > 0 && (
        <motion.div
          ...
          className="px-6 py-3 border-b border-slate-200 dark:border-slate-700"
        >
          <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Cuenta activa</label>
          <div className="relative">
            <button
              onClick={() => setShowAccountSelector(!showAccountSelector)}
              className="w-full flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
            >
              <div className="flex items-center space-x-2">
                <IconWallet className="w-4 h-4 text-slate-400 dark:text-slate-500" />
                <span className="font-mono text-slate-800 dark:text-slate-100">
                  {selectedCuenta?.numeroCuenta || 'Seleccionar'}
                </span>
              </div>
              <IconChevronDown className={clsx('w-4 h-4 text-slate-400 dark:text-slate-500 transition-transform', showAccountSelector && 'rotate-180')} />
            </button>
            ...
            <AnimatePresence>
              {showAccountSelector && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute z-30 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg overflow-hidden"
                >
                  {cuentas.map((cuenta) => (
                    <button
                      key={cuenta.id}
                      ...
                      className={clsx(
                        'w-full text-left px-3 py-2 text-sm hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors',
                        cuenta.id === selectedCuentaId ? 'bg-blue-50 dark:bg-slate-700 text-blue-900 dark:text-blue-200' : 'text-slate-700 dark:text-slate-200'
                      )}
                    >
      ...
      >
        <button
          ...
          className="flex items-center space-x-3 w-full px-3 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-all"
        >
